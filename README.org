...
The repository includes a CI pipeline that runs on every push and pull
request to the =main= branch.  It byte-compiles the package and
executes the ERT test suite (=pangram-test.el=) against Emacs 29.4 and
30.1.
...
All HTTP calls in the test suite are mocked, so no API key or network
access is needed to run the tests.
